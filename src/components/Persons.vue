<template>
  <v-container grid-list-xs>
      <v-toolbar flat>
        ...
        <span class="grey--text caption" v-if="isOpened"><v-icon color="success">mdi-emoticon</v-icon></span>
        <span class="grey--text caption" v-if="!isOpened"><v-icon color="error">mdi-emoticon-sad</v-icon></span>
        <v-btn class="text-lowercase" text @click="dialog = !dialog">
        ...
        <v-layout row wrap>
          <v-flex xs12 sm6 md4 lg3 v-for="(person, index) in person" :key="index">
            <v-card class="ma-4 text-center pa-3" dark>
              ...
              <br>
              <span class="caption">{{ person.age }}</span>
              <br>
